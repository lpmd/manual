...
Uno de los requerimientos b\'asicos de \lpmd es tener bien configurada la ubicaci\'on de los plugins que \lpmd requiere, es por eso que se debe utilizar la ubicaci\'on de la instalaci\'on de la librer\'ia lpmd \textbf{liblpmd}. Usualmente deber\'ia correrse con:
...
Como ya se especific\'o, existen dos tipos de ficheros con posiciones at\'omicas, uno en coordenadas cartesianas (\textbf{.xyz}) y el otro en coordenadas fraccionarias (\textbf{.lpmd}). Para ingresarlos en el fichero de configuraci\'on se debe especificar el m\'odulo del fichero con posiciones at\'omicas que utilizaremos y el nivel que tiene.

\cajatx{ 
\begin{tabular}{lcl}
 module & = & indica el m\'odulo para cargar la celda,\\ 
&&\textbf{xyz} y \textbf{lpmd}.\\
 file & = & indica el fichero con posiciones at\'omicas,\\
&& \textbf{.xyz} y \textbf{.lpmd}.\\
 level & = & indica el nivel del fichero,\\ 
&&\textbf{0}=posiciones; \textbf{1}=posiciones y velocidad; \\
&& \textbf{2}=posiciones, velocidades y aceleraciones.\\
\end{tabular}
}
