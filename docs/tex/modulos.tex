...
Con el m\'odulo \verb|dlpoly|, se pueden cambiar los archivos de configuracion de \verb|dlpoly| (\verb|HISTORY| o \verb|CONFIG|) a cualquier otro m\'odulo y viceversa. Los argumentos utilizados por el m\'odulo \verb|dlpoly| son:

\fb{
\begin{tabular}{lcl}
 file & = & Archivo que contiene la configuraci\'on de \textbf{dlpoly}\\
 each & = & Cada cuantos pasos se debe escribir el archivo.\\
 level & = & Se especifica el nivel del formato del archivo (0,1,2).\\
 && 0/1/2 $\leftrightarrow$ pos/pos-vel/pos-vel-acel.\\
 periodicity & = & Especifica \textit{key} de periodicidad del fichero CONFIG (1 o 0).\\
 ftype & = & Especif\'ica el tipo de archivo (\texttt{HISTORY} o \texttt{CONFIG}) a ser le\'ido.\\
\end{tabular}
}
...
\begin{tabular}{lcl}
 ...
 level & = & Se especifica el nivel del formato del archivo (0,1,2).\\
 && 0/1/2 $\leftrightarrow$ pos/pos-vel/pos-vel-acel.\\
 extra & = & Informaci\'on extra en el fichero sobre los \'atomos (RGB,C,TYPE).\\
 && RGB: Los \'atomos en el archivo poseen/tendr\'an colores en formato\\&& $<$red,green,blue$>$ en el archivo de entrada/salida.\\
 && C: Los \'atomos en el archivo poseen/tendr\'an colores en formato\\&& C$\in[0,1]$ en el archivo de entrada/salida.\\
 type & = & Especif\'ica el tipo de fichero (\texttt{lpmd}(default) o \texttt{zlp}).\\
 blocksize & = & Tamaño del \textit{block} de compresi\'on cuando el archivo es del tipo \texttt{zlp}.\\ 
\end{tabular}
}

La forma m\'as com\'un del formato lpmd es la siguiente:

\begin{itemize}
 \item \textbf{Cargando un fichero lpmd}
       \control{input module=lpmd file=archivo.lpmd}
 \item \textbf{Escribiendo la salida en un fichero lpmd con velocidades y colores}
       \control{output module=lpmd file=salida.lpmd level=1 extra=rgb}
\end{itemize}
...
\begin{tabular}{lcl}
 rcut & = & Radio de corte m\'aximo para an\'alisis.\\
 bins & = & N\'umero m\'aximo de divisiones entre 0 y \textbf{rcut}.\\
 output & = & Archivo de salida.\\
 average & = & Se promediar\'an o no los calculos (true/false).\\
\end{tabular}
}
